...
<html>
	...
	<body>
		...
		<form name='carSelect' id='carSelect' action='reservations.php' method='post'>
			<table border='0' class="box">
				<tr>
					<th colspan=3>Reservation Tracker</th>
				</tr>
				<tr>
					<td>
			...
			<table border='1' class="resbox"
				<?php
					...
					if($dispData) {
						...
						$query = "SELECT RentalID, MemNo, VIN, ResLength FROM reservation WHERE Date=?";
						...
						// Display results in a table
							echo '<tr><th>RentalID</th><th>Member Number</th><th>VIN</th><th>Length (Days)</th></tr>';
							while($row = $result->fetch_assoc()) {
								//$name = $row['Make'].', '.$row['Model'];
								echo '<tr><td>'.$row['RentalID'].'</td><td>'.$row['MemNo'].'</td><td>'.$row['VIN'].'</td><td>'.$row['ResLength'].'</td></tr>';
							}
							echo '<tr><th colspan=4>Reservations for '.$dt.'</th></tr>';
						} else {
							// Car has no history yet
							echo '<tr><th>There are no reservations for '.$dt.'</th></tr>';
						}
					}
